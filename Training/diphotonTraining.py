...
inFileName = "../../CMSSW_12_4_8/src/allPhotonsNTuples/GGH13andDataB_DiphotonTrain_0724.root"

outNamesGen = []
outNamesGen.append("0728/M9LR15_GGH13andData_M60_0728")

modelNamesB = []
modelNamesB.append("barrelOut/" + outNamesGen[0] + "_Barrel.model")
...
#
modelNamesE = []
modelNamesE.append("endcapOut/" + outNamesGen[0] + "_Endcap.model")
...
xLeadTrainB = xLeadTrain[np.abs(xLeadTrain[:,7]) < 1.5]
xSubTrainB = xSubTrain[np.abs(xSubTrain[:,7]) < 1.5]
xLeadTrainE = xLeadTrain[np.abs(xLeadTrain[:,7]) > 1.5]
xSubTrainE = xSubTrain[np.abs(xSubTrain[:,7]) > 1.5]

xLeadTestB = xLeadTest[np.abs(xLeadTest[:,7]) < 1.5]
xSubTestB = xSubTest[np.abs(xSubTest[:,7]) < 1.5]
xLeadTestE = xLeadTest[np.abs(xLeadTest[:,7]) > 1.5]
xSubTestE = xSubTest[np.abs(xSubTest[:,7]) > 1.5]

varsLeadTrainB = varsLeadTrain[np.abs(xLeadTrain[:,7]) < 1.5]
varsSubTrainB = varsSubTrain[np.abs(xSubTrain[:,7]) < 1.5]
varsLeadTrainE = varsLeadTrain[np.abs(xLeadTrain[:,7]) > 1.5]
varsSubTrainE = varsSubTrain[np.abs(xSubTrain[:,7]) > 1.5]

varsLeadTestB = varsLeadTest[np.abs(xLeadTest[:,7]) < 1.5]
varsSubTestB = varsSubTest[np.abs(xSubTest[:,7]) < 1.5]
varsLeadTestE = varsLeadTest[np.abs(xLeadTest[:,7]) > 1.5]
varsSubTestE = varsSubTest[np.abs(xSubTest[:,7]) > 1.5]

yLeadTrainB = yLeadTrain[np.abs(xLeadTrain[:,7]) < 1.5]
ySubTrainB = ySubTrain[np.abs(xSubTrain[:,7]) < 1.5]
yLeadTrainE = yLeadTrain[np.abs(xLeadTrain[:,7]) > 1.5]
ySubTrainE = ySubTrain[np.abs(xSubTrain[:,7]) > 1.5]

yLeadTestB = yLeadTest[np.abs(xLeadTest[:,7]) < 1.5]
ySubTestB = ySubTest[np.abs(xSubTest[:,7]) < 1.5]
yLeadTestE = yLeadTest[np.abs(xLeadTest[:,7]) > 1.5]
ySubTestE = ySubTest[np.abs(xSubTest[:,7]) > 1.5]

wLeadTrainB = wLeadTrain[np.abs(xLeadTrain[:,7]) < 1.5]
wSubTrainB = wSubTrain[np.abs(xSubTrain[:,7]) < 1.5]
wLeadTrainE = wLeadTrain[np.abs(xLeadTrain[:,7]) > 1.5]
wSubTrainE = wSubTrain[np.abs(xSubTrain[:,7]) > 1.5]

wLeadTestB = wLeadTest[np.abs(xLeadTest[:,7]) < 1.5]
wSubTestB = wSubTest[np.abs(xSubTest[:,7]) < 1.5]
wLeadTestE = wLeadTest[np.abs(xLeadTest[:,7]) > 1.5]
wSubTestE = wSubTest[np.abs(xSubTest[:,7]) > 1.5]
...
modelB = XGBClassifier(max_depth = mdB, learning_rate = lrB, subsample = 0.9, max_delta_step = 0.0, min_child_weight = 0.0, reg_alpha = 0.0, reg_lambda = 4.0, colsample_bytree = 0.65, verbosity = 0, n_estimators=nEstB, nthread = 16, feature_names = inputVarNames)
evalSetB = [(xTrainB, yTrainB), (xTestB, yTestB)]
modelB.fit(xTrainB, yTrainB, sample_weight = wTrainB, eval_metric=["auc", "logloss"], eval_set=evalSetB, early_stopping_rounds=50, verbose=1000)

modelE = XGBClassifier(max_depth = mdE, learning_rate = lrE, subsample = 0.9, max_delta_step = 0.0, min_child_weight = 0.0, reg_alpha = 0.0, reg_lambda = 4.0, colsample_bytree = 0.65, verbosity = 0, n_estimators=nEstB, nthread = 16, feature_names = inputVarNames)
evalSetE = [(xTrainE, yTrainE), (xTestE, yTestE)]
modelE.fit(xTrainE, yTrainE, sample_weight = wTrainE, eval_metric=["auc", "logloss"], eval_set=evalSetE, early_stopping_rounds=50, verbose=1000)
