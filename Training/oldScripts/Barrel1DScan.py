...
import BarrelPtUtils
import BarrelTMVAUtils
...
fake = fin['bkgTree']
## for barrel
geometry_selection = lambda tree: np.abs(tree.array('eta')) < 1.4442
...
input_values, target_values, orig_weights, train_weights, pt, scEta, input_vars = BarrelPtUtils.load_file(fin,geometry_selection,ptCut,massCut)
...
for i in range(0,nModels):
    ...
    MD = mdVals[i]
    LR = lrVals[i]
    fNameGen = "barrelOut/Model_NewGGH_MD" + mdStrings[i] + "LR" + lrStrings[i] + "_M90_Barrel_0504"
    fName = fNameGen + ".npz"
...
#    model.fit(X_train, y_train, sample_weight = w_train, eval_metric=["logloss"], eval_set=eval_set, early_stopping_rounds=50, verbose=True)
...
#    fakeClass = myStruct()
#    tPrompt = TTree("sigPhotons", "GGH Signal")
#    tPrompt.Branch( 'XGBScores', promptClass, 'yVal/D:yPred/D' )
#    tFake = TTree("bkgPhotons", "Data Background")
...
#        if y_train[n] == 1:
#            sigVals.append(y_train_pred[n])
#        if y_train[n] == 0:
#            bkgVals.append(y_train_pred[n])
#
#    sigLen = len(sigVals)
#    bkgLen = len(bkgVals)
...
#    tBkg.Branch( 'passFailStd', passFailStdBkg, 'passFailStd/I' )
    ...
    BarrelTMVAUtils.variable_importance(model,input_variables,fNameGen)
