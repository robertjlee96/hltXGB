...
extraVars = ['et','energy']
singleVars = ['mass','nEgs','passFailStd','passFailL1Single','passFailL1Double']
...
#                    mask.append(False)
            ...
            if varname not in singleVars and varname != 'index':
                print [item[0] for item in tree.array(varname)]
                if label == 0:
                    varValuesBkgLead.append([item[0] for item in tree.array(varname)])
                    varValuesBkgSub.append([item[1] for item in tree.array(varname)])
                if label == 1:
                    varValuesSigLead.append([item[0] for item in tree.array(varname)])
                    varValuesSigSub.append([item[1] for item in tree.array(varname)])
...
#
                if varname in inputVars:
                    # BDT input variable
                    if label == 0:
                        inputValuesBkgLead.append([item[0] for item in tree.array(varname)])
                        inputValuesBkgSub.append([item[1] for item in tree.array(varname)])
                    if label == 1:
                        inputValuesSigLead.append([item[0] for item in tree.array(varname)])
                        inputValuesSigSub.append([item[1] for item in tree.array(varname)])
...
#                #thisWeights =  tree.array('weight')[indices]
    ...
    inputValuesSigLead = np.array(np.vstack(inputValuesSigLead[0]).T)
    varValuesSigLead = np.array(np.vstack(varValuesSigLead[0]).T)
