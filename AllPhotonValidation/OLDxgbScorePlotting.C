...
void xgbScorePlotting(){
    ...
    string fileName = "validationNTuples/0724/GGH13andData_MD09LR10_NoTrkIso_Validated.root";
    string genTitleStringSignal = "GGH Signal ";
    string genTitleStringBkg = "Data ";
    string dirStr ="sigBkgVarPlots/0724/NewData_OldModel_MD9LR10_M60_CoarseBin/";
    string plotType = "Added";
    string out2D = dirStr + "lowHighScore_XGBCut_0724";
    string outLead = dirStr + "lowScore_XGBCut_0724";
    string outSub = dirStr + "highScore_XGBCut_0724";
    ...
    while (inReaderSig.Next()) {
        ...
        if(inEtSig[iOutXGBSig] > 14.25 && inEtSig[jOutXGBSig] > 14.25 && abs(inEtaSig[iOutXGBSig]) < 2.5 && abs(inEtaSig[jOutXGBSig]) < 2.5 && iOutXGBSig != -1 && jOutXGBSig != -1 && massCalcXGBSig > 95.0){
            ...
            int e;
            if (abs(inEtaSig[iOutXGBSig]) < 1.5 && abs(inEtaSig[jOutXGBSig]) < 1.5)e = 0;
            if ((abs(inEtaSig[iOutXGBSig]) > 1.5 && abs(inEtaSig[jOutXGBSig]) < 1.5)
                || (abs(inEtaSig[iOutXGBSig]) < 1.5 && abs(inEtaSig[jOutXGBSig]) > 1.5))e = 1;
            if (abs(inEtaSig[iOutXGBSig]) > 1.5 && abs(inEtaSig[jOutXGBSig]) > 1.5)e = 2;
            
            hSigAllLead[e]->Fill(outOne);
            hSigAllSub[e]->Fill(outTwo);
            
            hSig2DScores[e]->Fill(outOne,outTwo);
            if((outOne > leadCuts1[e] && outTwo > subCuts1[e]) || (outOne > leadCuts2[e] && outTwo > subCuts2[e]) ||  (outOne < leadCuts2[e] && outTwo > subCuts3[e]))hSig2DScoresPass[e]->Fill(outOne,outTwo);
                
            if((outOne > leadCut && outTwo > subCutLow) || (outOne < leadCut && outTwo > subCutHigh)){
    ...
    while (inReaderBkg.Next()) {
        ...
        if(inEtBkg[iOutXGBBkg] > 14.25 && inEtBkg[jOutXGBBkg] > 14.25 && abs(inEtaBkg[iOutXGBBkg]) < 2.5 && abs(inEtaBkg[jOutXGBBkg]) < 2.5 && iOutXGBBkg != -1 && jOutXGBBkg != -1 && massCalcXGBBkg > 95.0){
            ...
            int e;
            if (abs(inEtaBkg[iOutXGBBkg]) < 1.5 && abs(inEtaBkg[jOutXGBBkg]) < 1.5)e = 0;
            if ((abs(inEtaBkg[iOutXGBBkg]) > 1.5 && abs(inEtaBkg[jOutXGBBkg]) < 1.5)
                || (abs(inEtaBkg[iOutXGBBkg]) < 1.5 && abs(inEtaBkg[jOutXGBBkg]) > 1.5))e = 1;
            if (abs(inEtaBkg[iOutXGBBkg]) > 1.5 && abs(inEtaBkg[jOutXGBBkg]) > 1.5)e = 2;
            
            hBkgAllLead[e]->Fill(outOne);
            hBkgAllSub[e]->Fill(outTwo);
            
            hBkg2DScores[e]->Fill(outOne,outTwo);
            if((outOne > leadCuts1[e] && outTwo > subCuts1[e]) || (outOne > leadCuts2[e] && outTwo > subCuts2[e]) ||  (outOne < leadCuts2[e] && outTwo > subCuts3[e]))hBkg2DScoresPass[e]->Fill(outOne,outTwo);
        ...
        if(inEtBkg[iOutHLTBkg] > 14.25 && inEtBkg[jOutHLTBkg] > 14.25 && abs(inEtaBkg[iOutHLTBkg]) < 2.5 && abs(inEtaBkg[jOutHLTBkg]) < 2.5 && iOutHLTBkg != -1 && jOutHLTBkg != -1 && massCalcHLTBkg > 95.0){
            //cout<<"For HLT, (i,j) = ("<<iOutHLTBkg<<","<<jOutHLTBkg<<"), mass = "<<massCalcHLTBkg<<endl;
            double outOne,outTwo;
            //outOne = inXGBBkg[iOutHLTBkg];
            //outTwo = inXGBBlg[jOutHLTBkg];
